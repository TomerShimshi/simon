...
def test_no_last_session_when_empty(tmp_path):
    store = MemoryProgressStore(storage_dir=tmp_path)
...
def test_persists_across_instances(tmp_path):
    store1 = MemoryProgressStore(storage_dir=tmp_path)
    store1.record_session(new_session_id(), pair_count=3, moves=4, mismatches=1)

    store2 = MemoryProgressStore(storage_dir=tmp_path)
    assert store2.last_session()["moves"] == 4


def test_best_moves_for_returns_none_when_no_sessions_at_that_size(tmp_path):
    store = MemoryProgressStore(storage_dir=tmp_path)
    assert store.best_moves_for(3) is None


def test_best_moves_for_tracks_fewest_per_grid_size(tmp_path):
    store = MemoryProgressStore(storage_dir=tmp_path)
